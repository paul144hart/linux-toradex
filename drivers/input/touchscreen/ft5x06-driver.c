...
//#define GPIO_TO_PIN(bank, nr)   (((bank) - 1) * 32 + (nr))
...
/*	if (ret < 0)
        ...
        if (ret == -5)
                pr_err("%s : Touch connection not detected : %d\n", __func__, ret);
        else if (ret == -11)
                pr_err("%s : Over current detected : %d\n", __func__, ret);
        else if (ret < 0)
                pr_err("%s : Touch connection issue : i2c read error : %d\n", __func__, ret);
        else
                pr_err("%s : Touch connection successful : %d\n", __func__, ret);
...
*/
//NOBEL : reset	
	err = gpio_request(43,FT5X0X_NAME);
...
*/	
//NOBEL : interrupt
	err = gpio_request(91,FT5X0X_NAME);
	gpio_direction_input(91);
	gpio_export(91,true);
	ct_irq_num = gpio_to_irq(91);
//NOBEL
