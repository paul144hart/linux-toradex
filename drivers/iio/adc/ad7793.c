...
/*Kiran Bhosale : Workaround for non-device tree  supported  ADC platform data */
...
//		goto out;
	...
	st->mode = AD7793_MODE_RATE(3); // Paul Grosshart - set to 3, which should be 4X settling time
...
{
	const struct ad7793_platform_data *pdata = spi->dev.platform_data = &ad7793_pdata; /*Kiran Bhosale : Manual Platform data support*/
	...
	if (pdata->refsel != AD7793_REFSEL_INTERNAL) {
		st->reg = devm_regulator_get(&spi->dev, "fixed-supply");
		if (IS_ERR(st->reg))
		...
		vref_mv /= 1000;
		printk("Vref = %d ", vref_mv);
	} else {
		vref_mv = 1170; /* Kiran bhosle  Build-in ref */
	}
