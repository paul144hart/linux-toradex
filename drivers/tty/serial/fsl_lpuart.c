...
static irqreturn_t lpuart_rxint(int irq, void *dev_id)
{
	...
	while (!(readb(sport->port.membase + UARTSFIFO) & UARTSFIFO_RXEMPT)) {
		...
		if (sr & (UARTSR1_PE | UARTSR1_OR | UARTSR1_FE)) {
			...
			if ((sr & sport->port.ignore_status_mask) && (port->tty->index != 2)) {
				if (++ignored > 100)
					goto out;
				continue;
			}
...
out:
	...
	tty_flip_buffer_push(port);
	/* 
		If we got a framing error, and the current FE count is more than 8
	   disable the receiver, clear the status
	*/
	if(sport->port.icount.frame>=8)&&(framing==true))
	{
		printk("Resetting UART...\n");
		cr2 = readb(sport->port.membase + UARTCR2);
		
		/* disable receiver */
		writeb(~UARTCR2_RE | cr2, sport->port.membase + UARTCR2);		
		/* clear overrun flag again */
		(void)readb(sport->port.membase + UARTSR1);
		(void)readb(sport->port.membase + UARTDR);
		/* flush RX FIFO */
		writeb(readb(sport->port.membase + UARTCFIFO) | UARTCFIFO_RXFLUSH,
		    sport->port.membase + UARTCFIFO);
		/* clear RDRF & OF status */
		writeb(UARTSFIFO_RXUF | UARTSFIFO_RXOF, sport->port.membase + UARTSFIFO);
		
		/* restore receiver RE to old value */
		writeb(cr2, sport->port.membase + UARTCR2);
		sport->port.icount.frame=0;
	}
	return IRQ_HANDLED;
}
...
//	printk("fsl_lpuart-brc baud=%d\n",baud);
//	printk("fsl_lpuart-brc clock=%ld\n",port->uartclk);
//	printk("fsl_lpuart-brc sbr=%ld\n",sbr);
//	printk("fsl_lpuart-brc brfa=%d\n",brfa);
	brfa &= UARTCR4_BRFA_MASK;
//	printk("fsl_lpuart-brc brfa=%d\n",brfa);
//	printk("fsl_lpuart-brc cr4=%d\n",cr4);
...
static int lpuart_probe(struct platform_device *pdev)
{
	...
	if (sport->lpuart32)
	{
		dev_info(&pdev->dev, "Using lpuart32...\n");		
	}
	else
	{
		dev_info(&pdev->dev, "Using lpuart...\n");		
	}
	...
	//if (!nodma) {
		dev_info(&pdev->dev, "Requesting DMA Channels...\n");
...
MODULE_DESCRIPTION("Freescale lpuart serial port driver");
MODULE_LICENSE("GPL v2");
>>>>>>> f3f5e6dea36680dc77ce374364556045c676f583
